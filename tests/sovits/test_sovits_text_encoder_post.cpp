// tests/sovits/test_sovits_text_encoder_post.cpp
...
static const std::string kModelF16 =
    kTestDir + "models/v2-text-encoder-post-f16.gguf";
static const std::string kRefDir = kTestDir + "ref/";
static const std::string kRefInputNpy = kRefDir + "v2_text_encoder_encoder2_input.npy";
static const std::string kRefOutputNpy = kRefDir + "v2_text_encoder_encoder2_output.npy";
...
static constexpr int64_t kLayers = 3;
...
static void fill_input(std::vector<float> & data) {
    for (size_t t = 0; t < static_cast<size_t>(kTime); ++t) {
        for (size_t c = 0; c < static_cast<size_t>(kHidden); ++c) {
            const size_t idx = t * static_cast<size_t>(kHidden) + c;
            data[idx] = std::sin(static_cast<float>(c) * 0.017f)
                      + std::cos(static_cast<float>(t) * 0.13f)
                      + 0.001f * static_cast<float>((c + t) % 11);
        }
    }
}
...
TEST(SoVITSTextEncoderPost, LoadsSuccessfully) {
    ASSERT_MODEL_EXISTS(kModelF16);

    ggml_backend_t backend = create_test_backend();
    ASSERT_NE(backend, nullptr);

    gpt_sovits::sovits_text_encoder_post_model model{};
    ASSERT_TRUE(gpt_sovits::sovits_text_encoder_post_model_load(kModelF16, model, backend));
    EXPECT_NE(model.backend, nullptr);
    EXPECT_NE(model.buf_w, nullptr);
    EXPECT_NE(model.ctx_w, nullptr);

    gpt_sovits::sovits_text_encoder_post_model_free(model);
    ggml_backend_free(backend);
}

TEST(SoVITSTextEncoderPost, WeightPointersAndShapesLookCorrect) {
    ASSERT_MODEL_EXISTS(kModelF16);
...
TEST(SoVITSTextEncoderPost, BuildsGraphAndRunsInference) {
    ASSERT_MODEL_EXISTS(kModelF16);
    ...
    struct ggml_tensor * inp = ggml_new_tensor_2d(gctx, GGML_TYPE_F32, kHidden, kTime);
    ggml_set_name(inp, "post_in");
    ggml_set_input(inp);

    struct ggml_tensor * out =
        gpt_sovits::sovits_text_encoder_post_block_forward(gctx, inp, model.weights);
    ASSERT_NE(out, nullptr);
    ggml_set_name(out, "post_out");
    ggml_set_output(out);
    ...
    std::vector<float> input(static_cast<size_t>(kHidden * kTime));
    fill_input(input);
...
TEST(SoVITSTextEncoderPost, MissingModelFileFailsCleanly) {
    ggml_backend_t backend = create_test_backend();
    ASSERT_NE(backend, nullptr);

    gpt_sovits::sovits_text_encoder_post_model model{};
    EXPECT_FALSE(gpt_sovits::sovits_text_encoder_post_model_load(
...
TEST(SoVITSTextEncoderPost, FreeOnEmptyModelIsSafe) {
    gpt_sovits::sovits_text_encoder_post_model model{};
    gpt_sovits::sovits_text_encoder_post_model_free(model);
}

TEST(SoVITSTextEncoderPost, MatchesPythonReference) {
    ASSERT_MODEL_EXISTS(kModelF16);
    ...
    ASSERT_EQ(ref_input.shape[1], static_cast<size_t>(kHidden));
    ...
    const std::vector<float> packed_input =
        pack_bct_to_ggml(ref_input.data, ref_input.shape, kHidden);
    const std::vector<float> expected_output =
        pack_bct_to_ggml(ref_output.data, ref_output.shape, kHidden);
    ASSERT_EQ(packed_input.size(), static_cast<size_t>(kHidden * time));
    ...
    struct ggml_tensor * inp = ggml_new_tensor_2d(gctx, GGML_TYPE_F32, kHidden, time);
    ggml_set_name(inp, "post_in");
    ggml_set_input(inp);
    ...
    printf("[text_encoder_post parity] T=%lld max_abs=%.6f rmse=%.6f mean_abs=%.6f\n",
