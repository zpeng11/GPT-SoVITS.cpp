#!/usr/bin/env python3
"""Convert SoVITS v2 `enc_p.encoder2` weights to GGUF.

Usage:
    python convert_sovits_text_encoder_post_to_gguf.py <sovits_ckpt> [--output <path>] [--type f32|f16]

This converter is torch-free. It reads the checkpoint with `torch_ckpt_utils`
and exports only the tensors needed by `sovits_text_encoder_post_block_weights`.
"""
...
for i in range(3):
    TEXT_ENCODER_POST_MAP.extend([
        ...
        (f"text_encoder_post.layers.{i}.rel_k", f"enc_p.encoder2.attn_layers.{i}.emb_rel_k"),
        (f"text_encoder_post.layers.{i}.rel_v", f"enc_p.encoder2.attn_layers.{i}.emb_rel_v"),
...
def _fused_qkv(weights: dict[str, np.ndarray], layer_idx: int) -> tuple[np.ndarray, np.ndarray]:
    prefix = f"enc_p.encoder2.attn_layers.{layer_idx}"
...
def _packed_rel_k(weights: dict[str, np.ndarray], layer_idx: int) -> np.ndarray:
    rel_k = weights[f"enc_p.encoder2.attn_layers.{layer_idx}.emb_rel_k"]
    return rel_k[0].copy()


def _packed_rel_v_t(weights: dict[str, np.ndarray], layer_idx: int) -> np.ndarray:
    rel_v = weights[f"enc_p.encoder2.attn_layers.{layer_idx}.emb_rel_v"]
...
def convert(sovits_path: str, output_path: str, dtype_str: str) -> None:
    ...
    writer = gguf.GGUFWriter(output_path, "sovits_text_encoder_post")
    writer.add_string("sovits.block", "text_encoder_post")
    writer.add_string("sovits.version", str(cfg.get("version", "v2")))
    writer.add_uint32("sovits.text_encoder_post.hidden_dim", 192)
    writer.add_uint32("sovits.text_encoder_post.ffn_dim", 768)
    writer.add_uint32("sovits.text_encoder_post.n_head", 2)
    ...
    for i in range(3):
        ...
        writer.add_tensor(
            f"text_encoder_post.layers.{i}.qkv_w",
            qkv_w,
            raw_dtype=target_type if qkv_w.dtype == np.float16 else gguf.GGMLQuantizationType.F32,
        )
        n_converted += 1
        print(
            f"  [{n_converted:2d}] {'text_encoder_post.layers.' + str(i) + '.qkv_w':36s} <- fused q/k/v weights{'':18s} "
            f"{list(qkv_w.shape)!s:16s} {qkv_w.dtype}"
        )

        writer.add_tensor(
            f"text_encoder_post.layers.{i}.qkv_b",
            qkv_b,
            raw_dtype=gguf.GGMLQuantizationType.F32,
        )
        n_converted += 1
        print(
            f"  [{n_converted:2d}] {'text_encoder_post.layers.' + str(i) + '.qkv_b':36s} <- fused q/k/v bias{'':21s} "
            f"{list(qkv_b.shape)!s:16s} {qkv_b.dtype}"
        )

        writer.add_tensor(
            f"text_encoder_post.layers.{i}.rel_k",
            rel_k,
            raw_dtype=gguf.GGMLQuantizationType.F32,
        )
        n_converted += 1
        print(
            f"  [{n_converted:2d}] {'text_encoder_post.layers.' + str(i) + '.rel_k':36s} <- packed rel_k{'':27s} "
            f"{list(rel_k.shape)!s:16s} {rel_k.dtype}"
        )

        writer.add_tensor(
            f"text_encoder_post.layers.{i}.rel_v_t",
            rel_v_t,
            raw_dtype=gguf.GGMLQuantizationType.F32,
        )
        n_converted += 1
        print(
            f"  [{n_converted:2d}] {'text_encoder_post.layers.' + str(i) + '.rel_v_t':36s} <- packed rel_v_t{'':25s} "
            f"{list(rel_v_t.shape)!s:16s} {rel_v_t.dtype}"
        )

    for gguf_name, ckpt_name in TEXT_ENCODER_POST_MAP:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Convert SoVITS v2 text_encoder_post weights to GGUF"
    )
    parser.add_argument(
        "sovits_ckpt",
    ...
    parser.add_argument(
        ...
        "-o",
        default=None,
        help="Output GGUF file path (default: <ckpt_stem>-text-encoder-post-<type>.gguf)",
    )
    parser.add_argument(
        "--type",
        "-t",
    ...
    if args.output is None:
        ...
        args.output = f"{base}-text-encoder-post-{args.dtype}.gguf"
